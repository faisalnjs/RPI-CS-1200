...
void reverseFrames(std::list<GifFrame>& frames) {
	...
	// std::list<GifFrame>::iterator start = frames.begin();
	// std::list<GifFrame>::iterator end = frames.end();
	std::list<GifFrame>::reverse_iterator start = frames.rbegin();
	std::list<GifFrame>::reverse_iterator end = frames.rend();
	while (start != end) {
		--end;
		if (start == end) break;
		std::swap(*start, *end);
