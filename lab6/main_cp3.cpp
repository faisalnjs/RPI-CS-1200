...
// check point 3: complete this function, so as to reverse this std::list.
void reverseFrames(std::list<GifFrame>& frames) {
    std::cout << "Number of frames: " << frames.size() << std::endl;
    int numberOfFrames = frames.size();
	std::list<GifFrame>::iterator pos = frames.end();
    for (int i = 0; i < numberOfFrames; i++) {
        GifFrame frameToMove = frames.front();
        frames.pop_front();
        pos = frames.insert(pos, frameToMove);
    };
    std::cout << "Number of frames: " << frames.size() << std::endl;
}
