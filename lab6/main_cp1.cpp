...
void reverseFrames(std::vector<GifFrame>& frames) {
	...
	for (int currentFrame = 0; currentFrame < (numberOfFrames / 2); currentFrame++) {
		GifFrame frameToMove = frames[currentFrame];
		int target = numberOfFrames - currentFrame - 1;
		frames[currentFrame] = frames[target];
		frames[target] = frameToMove;
	};
