  ...
  for (unsigned int y = 0; y < blocked_grid[0].size(); ++y) {
    for (unsigned int x = 0; x < blocked_grid.size(); ++x) {
      if (x == start_x && y == start_y)
        ...
        std::cout << " X";
      else if (blocked_grid[x][y] == GRID_ACTIVE)
        std::cout << " $";
      else
        std::cout << " .";
    }
...
std::list<Point> firstPath;
bool firstPathFound = false;

int countPaths(const std::vector<std::vector<GRID_STATUS>>& blocked_grid,
               Point currentPoint, std::list<Point>& currentPath) {
  if (blocked_grid[currentPoint.x][currentPoint.y] == GRID_BLOCKED) return 0;
  ...
  if (currentPoint.x > 0) totalPaths += countPaths(blocked_grid, Point(currentPoint.x - 1, currentPoint.y), currentPath);
  if (currentPoint.y > 0) totalPaths += countPaths(blocked_grid, Point(currentPoint.x, currentPoint.y - 1), currentPath);
  currentPath.pop_back();
  return totalPaths;
}
...
int main(int argc, char* argv[]) {
  ...
  //   std::cout << blocked_grid[0][1] << std::endl;

  Point currentPoint(start_x, start_y);
  std::cout << "(" << start_x << ", " << start_y << ")" << std::endl;
  std::list<Point> currentPath;
  std::cout << "Paths found: " << countPaths(blocked_grid, currentPoint, currentPath) << std::endl;
  std::cout << "First path found? " << (firstPathFound ? "Yes" : "No") << std::endl;
  if (!firstPath.empty()) {
    std::cout << "First path found: " << std::endl;
    std::list<Point>::iterator p;
    for (p = firstPath.begin(); p != firstPath.end(); ++p) {
      std::cout << "(" << p->x << ", " << p->y << ")" << std::endl;
      blocked_grid[p->x][p->y] = GRID_ACTIVE;
    }
  }
  print_grid(blocked_grid, start_x, start_y);
