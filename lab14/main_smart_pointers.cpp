...
class BalloonCheckpoint2 { // Use this instead of Balloon for checkpoint 2
public:
  // CONSTRUCTOR & DESTRUCTOR
  BalloonCheckpoint2(const std::string& name_) : name(name_) {
    std::cout << "Balloon constructor " << name << std::endl;
    num_ropes = 0;
...
std::set<const Balloon*> visited;
void deleteAll(const Balloon* balloon) {
  if (balloon == NULL) return;
  if (visited.find(balloon) != visited.end()) return;
  std::cout << balloon->getName() << " has " << balloon->numRopes() << " ropes" << std::endl;

  for (int i = 0; i < balloon->numRopes(); i++) {
    visited.insert(balloon);
    deleteAll(balloon->getRope(i));
  }
...
//   georgette->removeRope(isabelle);
//   georgette->removeRope(henry);
//   georgette->removeRope(henry);
