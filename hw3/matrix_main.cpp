...
void StudentTest(){
  Matrix oneMatrix;                                                                          // Initialize oneMatrix
  assert((oneMatrix.num_rows() == 0) && (oneMatrix.num_cols() == 0));                        // Confirm oneMatrix has 0 rows and 0 columns
  Matrix anotherMatrix(12, 34, 56);                                                          // Initialize anotherMatrix with 12 rows, 34 columns, and all values set to 56
  assert((anotherMatrix.num_rows() == 12) && (anotherMatrix.num_cols() == 34));              // Confirm anotherMatrix has 12 rows and 34 columns
  double doubleToCompareTo;                                                                  // Initialize doubleToCompareTo
  anotherMatrix.get(11, 22, doubleToCompareTo);                                              // Store the value at row 11 and column 22 of anotherMatrix in doubleToCompareTo
  assert(doubleToCompareTo);                                                                 // Confirm doubleToCompareTo exists
  assert(double_compare(doubleToCompareTo, 0) == false);                                     // Confirm doubleToCompareTo is not 0
  assert(double_compare(doubleToCompareTo, 56));                                             // Confirm doubleToCompareTo is 56
  assert(oneMatrix != anotherMatrix);                                                        // Confirm that oneMatrix and anotherMatrix are not equal
  Matrix finalMatrix;                                                                        // Initialize finalMatrix
  assert(oneMatrix == finalMatrix);                                                          // Confirm that oneMatrix and finalMatrix are equal
  finalMatrix = anotherMatrix;                                                               // Set finalMatrix equal to anotherMatrix
  assert(oneMatrix != finalMatrix);                                                          // Confirm that finalMatrix and oneMatrix are not equal
  assert(anotherMatrix == finalMatrix);                                                      // Confirm that finalMatrix and anotherMatrix are equal
  finalMatrix.multiply_by_coefficient(1234.5);                                               // Multiply finalMatrix by double 1234.5
  assert(anotherMatrix != finalMatrix);                                                      // Confirm that finalMatrix and anotherMatrix are not equal
  finalMatrix.set(11, 22, 12345);                                                            // Set row 11 and column 22 of finalMatrix to 69132
  finalMatrix.swap_row(11, 22);                                                              // Swap col 11 and col 22 of finalMatrix
  finalMatrix.transpose();                                                                   // Transpose finalMatrix
  assert(anotherMatrix != finalMatrix);                                                      // Confirm that finalMatrix and anotherMatrix are not equal
  double* colToCompareTo = finalMatrix.get_col(11);                                          // Get col 11 of finalMatrix and store it in colToCompareTo
  assert(colToCompareTo != NULL);                                                            // Confirm colToCompareTo exists
  assert(colToCompareTo[22]);                                                                // Confirm row 22 in colToCompareTo exists
  assert(colToCompareTo[22] == 12345);                                                       // Confirm colToCompareTo is 69132
  Matrix zeroMatrix(0, 0, 0);                                                                // Initialize zeroMatrix with 0 rows and 0 columns
  assert((zeroMatrix.num_rows() == 0) && (zeroMatrix.num_cols() == 0));                      // Confirm zeroMatrix has 0 rows and 0 columns
  Matrix oddMatrix(3, 3, 1);                                                                 // Initialize oddMatrix with 3 rows and 3 columns, and all values set to 1
  assert((oddMatrix.num_rows() == 3) && (oddMatrix.num_cols() == 3));                        // Confirm oddMatrix has 3 rows and 3 columns
  Matrix* oddMatrixQuarters = oddMatrix.quarter();                                           // Quarter oddMatrix and store the result in oddMatrixQuarters
  assert(oddMatrixQuarters);                                                                 // Confirm oddMatrixQuarters exists
  assert((oddMatrixQuarters[0].num_rows() == 2) && (oddMatrixQuarters[0].num_cols() == 2));  // Confirm UL of oddMatrix has 2 rows and 2 columns
  assert((oddMatrixQuarters[1].num_rows() == 2) && (oddMatrixQuarters[1].num_cols() == 2));  // Confirm UR of oddMatrix has 2 rows and 2 columns
  assert((oddMatrixQuarters[2].num_rows() == 2) && (oddMatrixQuarters[2].num_cols() == 2));  // Confirm LL of oddMatrix has 2 rows and 2 columns
  assert((oddMatrixQuarters[3].num_rows() == 2) && (oddMatrixQuarters[3].num_cols() == 2));  // Confirm LR of oddMatrix has 2 rows and 2 columns
  for (unsigned int i = 0; i < 4; i++) {                                                     // For each quarter of oddMatrix...
    for (unsigned int j = 0; j < oddMatrixQuarters[i].num_rows(); j++) {                     // For each row in the quarter...
      for (unsigned int k = 0; k < oddMatrixQuarters[i].num_cols(); k++) {                   // For each column in the quarter...
        double valueToCompare;                                                               // Initialize valueToCompare
        oddMatrixQuarters[i].get(j, k, valueToCompare);                                      // Get the value at the current row and column in the quarter and store it in valueToCompare
        assert(double_compare(valueToCompare, 1));                                           // Confirm valueToCompare is 1
      }
    }
  }
  std::cout << "oneMatrix:";
  std::cout << oneMatrix << std::endl;
  std::cout << "anotherMatrix:";
  std::cout << anotherMatrix << std::endl;
  std::cout << "finalMatrix:";
  std::cout << finalMatrix << std::endl;
  delete[] colToCompareTo;     // Clean up colToCompareTo
  delete[] oddMatrixQuarters;  // Clean up oddMatrixQuarters
}
